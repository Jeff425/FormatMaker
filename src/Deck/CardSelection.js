    ...
    return (
      <div className="AppContainer">
        ...
        {this.state.showInfo && (<Card className="fullWidth">
          <Card.Body>
            <Card.Title className="mb-1">{this.props.formatName}</Card.Title>
            {this.props.commanderFormat && <Card.Subtitle className="text-muted mb-2">Commander Format</Card.Subtitle>}
                    ...
                    <div className="fullWidth centerAlign">
                      {cards && cards.slice((this.state.page - 1) * 60, this.state.page * 60).map(card => 
                        <CardObj 
                          ...
                          onSide={this.props.sideboardAllowed ? card => this.props.addCard(card, true) : null} 
                          usePointSystem={group.usePointSystem} 
                          addCommander={(this.props.commanderFormat && !(card.name in this.props.deckAmount) && !(card.name in this.props.sideAmount)) ? this.props.addCommander : null} />
                      )}
                    </div>)}
